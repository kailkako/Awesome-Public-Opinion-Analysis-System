...
def main(articleCommentsFilePath):
    ...
    # text_loader = DataLoader(text_dataset, batch_size=80, num_workers=24)  # 用gpu时，可以用这行代码，使用多个工作进程
    text_loader = DataLoader(text_dataset, batch_size=1)  # 用cpu时，就不要使用多个工作进程，不然报错
